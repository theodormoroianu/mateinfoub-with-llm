...
def convert_statements(force: bool = False):
    ...
    if not ro_statements.exists():
        print(
            f"Could not find {ro_statements}. Please create the file and add data (for instance from https://mateinfo-ub.github.io/#/toate-datele)."
        )
        sys.exit(0)

    if en_statements.exists() and not force:
        print(
            f"{en_statements} already exists. If you want to overwrite it, use --force."
        )
        sys.exit(0)
